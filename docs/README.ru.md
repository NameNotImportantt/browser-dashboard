...
См. [руководство разработчика](./DEVELOPERS.ru.md) — сборка, структура проекта и чеклист релиза. Текущая схема сборки разделена: `bun run build` делает single-file release-артефакт, а `bun run build:multi` собирает обычный multi-file Vite output с PWA-поддержкой для `https://` и `http://localhost`. Чтобы отправить pull request, см. [Contributing](../CONTRIBUTING.md) (на английском).
