...
function suggestionSourceLabel(locale: SearchCoreProps["locale"], source: SearchSuggestion["source"]) {
  if (source === "history") {
...
export function SearchCore({
  ...
  const openSearch = useCallback(
    (searchQuery: string) => {
      ...
      void onAddSearchHistory(trimmed).catch(() => undefined);
      const url = buildSearchUrl(activeSearchEngineId, trimmed, customSearchEngines);
      window.open(url, "_blank", "noopener,noreferrer");
      setQuery("");
      closeSuggestions();
      inputRef.current?.focus();
    },
    [activeSearchEngineId, closeSuggestions, customSearchEngines, onAddSearchHistory],
  );
