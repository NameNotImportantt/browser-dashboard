...
export function QuickLinks({ bookmarks, categories, locale, onAdd, onDelete, onAddCategory, onDeleteCategory }: QuickLinksProps) {
  ...
  const submitLink = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await onAdd({ title, url, categoryId: activeCategoryId });
    setTitle("");
    setUrl("");
  ...
  const submitCategory = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    await onAddCategory({ name: categoryName });
    setCategoryName("");
    ...
    <section className={styles.quickLinks} aria-label={t(locale, "savedLinks")}>
      ...
      {isAddingLink ? (
        <form className={styles.addForm} onSubmit={submitLink}>
          <input value={title} onChange={event => setTitle(event.target.value)} placeholder={t(locale, "bookmarkTitle")} required />
